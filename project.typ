...
= Technical abstract
Large Language Models (LLms) have become important component in modern software and continues to help businesses in many fields. Practical adoption however has been limited by absence of standardized, secure and scalable mechanisms for interacting with extenal tools and services. The Machine Context Protocol (MCP) was developed to address this limitation by providing structured and developer friendly way of making software capable of communicating trough api calls. While MCP significantly improves tool integration, early versions of the protocol showed notable shortcomings mainly in areas of authentication, session management and monetization.

Problem this project investigates is functionality of the Machine Context Protocol from server-side perspective, with focus on it's integration with existing api based applications that rely on user authentication and paid access models. The main objective is to analyze and demonstrate how a traditional API service can be transformed into an MCP-capable application without sacrificing existing functionality.

Practical case study will be done on Adversea application with aim to transform it from paid api service into Mcp-enabled system. Examined will also be the role of MCP Gateway as a reverse proxy that enables authentication trough Oauth standard, session management and secure communication between MCP Client and MCP Server. In addition the project also explores the integration of x402, a recently introduced protocol to obsolve internet of it' absence of internet native way for transaction processing. With use of the x402 standard the project looks into how per request payments can change project  not only allowing users to pay without registration but also to allow agent-to-service and agent-to-agent payments.

The expected outcome of this work is a reproducible approach for integrating MCP, MCP Gateway, and x402 into authentication and monetization requiring API services. This project aims to contribute practical insights into the quickly evolving MCP ecosystem and provide guidance to developers and businesses who are looking to adopt MCP in their applications.

= Dummy abstract
In recent years artificial intelligence models capable of generating human language have become importnat part and are currently used in many digital products and services. Despite their advances the models have always faced issues with usage of external applications especially those that require payments for their services. This limitation has drastically reduced their usefullness for real world businesses.

To address this limitation a new technology called Machine Context Protocol was invented to help Large Language Models communicate more effectively with online services. While this significantly improved how systems could access external tools, it initially lacked needed features such as authentication and transaction processing. To solve these problems additional solutiosn such as MCP Gateway and x402 were developed.

This project explores how an existing online services can be adapted to work with these new technologies. Focus will be put on transforming the Adversea application from tradition paid service into a system that can interact with artificial intelligence directly in a secure way. Attention will be also given to enabling user authentication and supporting pay-per-use access without requiring prio registration.

By examining these technologies, this project seeks to demonstrate how Large Language Models may be can be more easily and securely connected to real world services. Results from this project are intended to help developers and businesses understand how MCP could simplify access to their services but also enable new ways of offering their online services.

= Introduction
Large language models have been the movingstone of society for the past 5 years. Their introduction brought us many advantages but for a very long time they had one fault and that is the use of third party tools. Presented solutions have not been the best for one reason or other till the Machine Context Protocol was introduced in November of 2024. Machine context protocol is a new field which is evolving quickly because of it's potential. It's way of adding llm the ability to interact with tools is unique from approaches used till now and it's simplicity has been the driving force behind it's popularity. The protocol itself being developed a year ago makes it an exciting new topic for both ai enthusiasts as well technical people in all fields. My project takes a deeper look into the protocols usage with heavy focus on server side. Task I aim to accomplish with this project is to explain how an api project may take advatage of this protocol and what it can gain. Discussed in later will be not only implemntation but also newest project mcp gateway and projects interacting with this protocol such as x402. My aim is to change an existing application Adversea from a paid api service to an application able to respond to mcp calls but also use all of it's best features such as already mentioned mcp gateway. As there are many paid services on the internet many of which could benefit from the machine context protocol I will aim to present everything in a reproducible manner so that anyone may use this as guide to enhance their own free or paid tools.
